...
function renderTemplateFiles (skipInterpolation) {
  ...
  return (files, metalsmith, done) => {
    ...
    async.each(keys, (file, next) => {
      ...
      if (!/{{([^{}]+)}}/g.test(str) && file !== 'package.json') {
        return next()
      }
...
function generatePackageJson(res){
  ...
  let {name,version,author,repository_type,repository_url,bugs_url,homePage,description} = global['talentui_info'];
  contentJson.name=name;
  contentJson.version=version;
  contentJson.author=author;
  contentJson.repository_type={
    repository_type,
    repository_url
  };
  contentJson.bugs={
